...
const DetailBaliTrek = () => {
  ...
  const fetchTourData = async () => {
    try {
      const tourResponse = await axios.get(`${BASE_URL}/package-tour/40`); // Ganti ID sesuai backend
      ...
      const galleryResponse = await axios.get(`${BASE_URL}/tour/gallery/40`);
      setGalleryImages(
        galleryResponse.data.map(img => ({ imgelink: `${BASE_URL}${img.img}` }))
      );

      // Fetch rundown
      const rundownResponse = await axios.get(`${BASE_URL}/tour/rundown/40`);
      setRundownData(
      ...
      <div className="flex items-center justify-center h-screen bg-white">
        ...
        <>
          ...
          <motion.div className="mt-6 md:mt-12 lg:mt-8 mx-auto px-4 text-center" variants={riseVariants}>
            ...
            {packageName}{" "} <span className="font-poppins text-customGreenslow font-semibold">Balinese Ancient Village Structure</span>
            </h1>
