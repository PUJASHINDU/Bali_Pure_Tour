...
const DetailBaliCook = () => {
  const [tourData, setTourData] = useState({
    title: { first: "Bali Pure Cook", second: "" },
  ...
  const fetchTourDetail = async () => {
    try {
      const BASE_URL = "http://localhost:5000"; // Pastikan URL sesuai dengan backend

      const response = await axios.get(`${BASE_URL}/package-tour/11`);
      ...
      const galleryResponse = await axios.get(`${BASE_URL}/tour/gallery/11`);
      if (galleryResponse.data && Array.isArray(galleryResponse.data)) {
        setGalleryImages(
          galleryResponse.data.map(img => ({
            imgelink: `${BASE_URL}${img.img}`, // Sesuaikan URL dengan backend
          }))
        );
      } else {
        setGalleryImages([]);
      ...
      // Fetch rundown
      const rundownResponse = await axios.get(`${BASE_URL}/tour/rundown/11`);
      setRundownData(
        ...
        <>
          ...
          <motion.div className="mt-6 md:mt-12 lg:mt-8 mx-auto px-4 text-center" variants={riseVariants}>
            <h1 className="font-poppins text-customGreen text-lg md:text-xl font-semibold">
              {packageName}{" "} <span className="font-poppins text-customGreenslow font-semibold">Balinese authentic cooking experience</span>
            </h1>
