...
// export const Register = async (req, res) => {
//   const { name, email, phone_number, password, role = "user" } = req.body; // Default "user"
//   const salt = await bcrypt.genSalt();
//   const hashPassword = await bcrypt.hash(password, salt);
...
//     const accessToken = jwt.sign({ id, name, email, role }, process.env.ACCSESS_TOKEN_SECRET, {
//       expiresIn: '1h',
//     });
...
//     res.json({ accessToken, user: { id, name, email, role } });
//   } catch (error) {
